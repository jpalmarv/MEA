...
//---- declarar constantes ---
const double K=1.0e2;               // Constantes de simulación (Cambiar con viscosidad)
...
const int Ntot = 329 ; // N pepas en líquido, Npepas pepas en sierras, 2 paredes)
...
int main(void){
    ...
    for(t=0,tdibujo=0 ; t<2*tmax; t+=dt,tdibujo+=dt){
        if(tdibujo>tcuadro){
            for (i = 0; i< N; i++){
            myfile << Molecula[i].Getx() << " " << Molecula[i].Gety() << '\n';
            }
